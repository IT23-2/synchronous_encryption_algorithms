def break_cipher(text):
    ...
    # Создаем множество допустимых символов для русского языкаAdd commentMore actions
    russian_letters = set(
...
max_score = 0Add commentMore actions
    decrypted_text = ''
    probable_key = 0

for k in range(65536):Add commentMore actions
        # Дешифруем текст с текущим ключом
        attempted_text = ''.join(
            [chr((ord(c) - k) % 65536) for c in text]
        )Add commentMore actions

 # Считаем количество допустимых символов в расшифрованном текстеAdd commentMore actions
